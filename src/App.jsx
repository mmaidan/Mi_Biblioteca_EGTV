      ...
      <main className="max-w-5xl mx-auto px-6 sm:px-10 py-8">
        ...
        <div className="flex flex-col gap-3 mb-6">
          <div className="flex gap-2">
            ...
            <div className="flex rounded-sm border shrink-0 overflow-hidden" style={{ borderColor: "#2E1A4733" }}>
              <button
                onClick={() => setViewMode("grid")}
                title="Ver como tarjetas"
                aria-label="Ver como tarjetas"
                className="px-3 py-2"
                style={{ background: viewMode === "grid" ? "#2E1A47" : "#FFFCFE", color: viewMode === "grid" ? "#FFFCFE" : "#241830" }}
              >
                <LayoutGrid size={14} />
              </button>
              <button
                ...
                className="px-3 py-2"
                style={{ background: viewMode === "list" ? "#2E1A47" : "#FFFCFE", color: viewMode === "list" ? "#FFFCFE" : "#241830" }}
              >
                <List size={14} />
              </button>
            </div>
...
function BookListRow({ book, perms, currentUser, schoolLabel, editorialLabel, onOpen, onLend, onReturn, onRequest, onCancelRequest }) {
  const spine = spineColorFor(book.id);
  const overdue = book.status === "loaned" && book.loan && daysUntil(book.loan.dueDate) < 0;
  const dueSoon = isDueSoon(book);
  const isMyRequest = book.status === "requested" && book.request && currentUser && book.request.studentName.toLowerCase() === currentUser.name.toLowerCase();
  ...
  return (
    <div className="flex items-center gap-3 rounded-sm border px-3 py-2.5" style={{ background: "#FFFCFE", borderColor: "#2E1A4722" }}>
      {book.coverUrl ? (
        <img src={book.coverUrl} alt="" className="w-8 h-11 object-cover rounded-sm shrink-0" />
      ) : (
        <div style={{ background: spine, width: 6, alignSelf: "stretch", borderRadius: 2 }} />
      )}
      <button onClick={onOpen} className="flex-1 min-w-0 text-left flex items-center gap-3">
        ...
        <div className="shrink-0">
          {book.status === "available" && (
            <span className="font-mono text-[10px] px-2 py-1 rounded-sm uppercase tracking-wide" style={{ background: "#2E1A4714", color: "#2E1A47" }}>
              Disponible
            </span>
          )}
          {book.status === "requested" && book.request && (
            <span className="font-mono text-[10px] px-2 py-1 rounded-sm uppercase tracking-wide" style={{ background: "#A66DD422", color: "#2E1A47" }}>
              Solicitado
            </span>
          )}
          {book.status === "loaned" && book.loan && (
            <span
              ...
              style={{ background: overdue ? "#7A2E6B22" : dueSoon ? "#9B2FAE22" : "#A66DD422", color: overdue ? "#7A2E6B" : dueSoon ? "#9B2FAE" : "#2E1A47" }}
            >
              {overdue ? "Atrasado" : dueSoon ? "Vence pronto" : "Prestado"}
            </span>
          )}
        </div>
      </button>
      <div className="flex items-center gap-1.5 shrink-0">
        {book.status === "available" && perms.request && (
          <button onClick={onRequest} className="font-mono text-[11px] uppercase tracking-wide px-2.5 py-1.5 rounded-sm" style={{ background: "#7A2E6B", color: "#FFFCFE" }}>
            Solicitar
          </button>
        )}
        {book.status === "available" && perms.lend && (
          <button onClick={onLend} className="font-mono text-[11px] uppercase tracking-wide px-2.5 py-1.5 rounded-sm" style={{ background: "#2E1A47", color: "#FFFCFE" }}>
            Prestar
          </button>
        )}
        {book.status === "requested" && perms.lend && (
        ...
        {book.status === "requested" && !perms.lend && isMyRequest && (
          <button onClick={onCancelRequest} className="font-mono text-[11px] uppercase tracking-wide px-2.5 py-1.5 rounded-sm border" style={{ borderColor: "#2E1A47", color: "#2E1A47" }}>
            Cancelar
          </button>
        )}
        {book.status === "loaned" && perms.return && (
          <button onClick={onReturn} className="font-mono text-[11px] uppercase tracking-wide px-2.5 py-1.5 rounded-sm border" style={{ borderColor: "#2E1A47", color: "#2E1A47" }}>
            Devolver
          </button>
        )}
      </div>
...
function GroupListRow({ copies, schoolLabel, editorialLabel, onOpenCopies }) {
  const first = copies[0];
  const spine = spineColorFor(groupKeyForStatic(first));
  const withCover = copies.find((c) => c.coverUrl);
  const available = copies.filter((c) => c.status === "available").length;
  ...
  return (
    <button onClick={onOpenCopies} className="flex items-center gap-3 rounded-sm border px-3 py-2.5 text-left" style={{ background: "#FFFCFE", borderColor: "#2E1A4722" }}>
      {withCover ? (
        <img src={withCover.coverUrl} alt="" className="w-8 h-11 object-cover rounded-sm shrink-0" />
      ) : (
        <div style={{ background: spine, width: 6, alignSelf: "stretch", borderRadius: 2 }} />
      )}
      <div className="min-w-0 flex-1">
        <p className="font-display font-semibold truncate">{first.title}</p>
        <p className="text-xs opacity-60 truncate">
          {first.author} · {first.genre}
          {schoolLabel && ` · ${schoolLabel}`}
          {editorialLabel && ` · ${editorialLabel}`}
        </p>
      </div>
      <span className="font-mono text-[10px] uppercase tracking-widest px-2 py-1 rounded-sm shrink-0" style={{ background: "#A66DD4", color: "#2E1A47" }}>
        {copies.length} ejemplares
      </span>
