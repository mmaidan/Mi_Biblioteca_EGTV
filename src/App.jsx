...
const PERMS = {
  admin: { addBook: true, lend: true, return: true, deleteBook: true, manageUsers: true, manageSchools: true, request: false },
  docente: { addBook: true, lend: true, return: true, deleteBook: false, manageUsers: false, manageSchools: false, request: false },
  alumno: { addBook: false, lend: false, return: false, deleteBook: false, manageUsers: false, manageSchools: false, request: true },
};
...
export default function Biblioteca() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [booksRes, usersRes, schoolsRes] = await Promise.allSettled([
        ...
        setSchools(schoolsRes.status === "fulfilled" && schoolsRes.value ? JSON.parse(schoolsRes.value.value) : DEFAULT_SCHOOLS);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    window.storage.set(SCHOOLS_KEY, JSON.stringify(schools), true).catch((e) => console.error(e));
  }, [schools, loading]);
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [visibleBooks, query, filter, genreFilter, schoolFilter, currentUser]);
  ...
  if (loading || books === null || users === null || schools === null) {
    return (
  ...
  return (
    <div style={{ background: "#F5EEFB", minHeight: "100vh", fontFamily: "'Source Serif 4', Georgia, serif", color: "#241830" }}>
      ...
      {showAdd && perms.addBook && <AddBookModal schools={schools} onClose={() => setShowAdd(false)} onSave={addBook} />}
...
function BookCard({ book, perms, currentUser, schoolLabel, onOpen, onLend, onReturn, onRequest, onCancelRequest, onDelete }) {
...
function AddBookModal({ schools, onClose, onSave }) {
  ...
  function submit() {
    ...
    onSave({ title, author, genre: finalGenre, condition, notes, schoolId });
  }
  ...
  return (
    <Modal onClose={onClose} title="Nuevo libro">
      ...
      <button
        onClick={submit}
        disabled={!title.trim() || !author.trim() || !schoolId}
...
function DetailModal({ book, perms, currentUser, schoolLabel, onClose, onDelete, onReturn, onLend, onRequest, onCancelRequest, onRate }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const overdue = book.status === "loaned" && book.loan && daysUntil(book.loan.dueDate) < 0;
  const isMyRequest = book.status === "requested" && book.request && currentUser && book.request.studentName.toLowerCase() === currentUser.name.toLowerCase();
  const rating = avgRating(book);
